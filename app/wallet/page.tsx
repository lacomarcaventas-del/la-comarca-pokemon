...
export default function WalletPage() {
  ...
  const [from,setFrom] = useState(""); const [to,setTo] = useState(""); const [amount,setAmount] = useState(""); const [note,setNote] = useState(""); const [message,setMessage] = useState("Cargando wallet…"); const [busy,setBusy] = useState(false);
  ...
  async function transfer(e:React.FormEvent){
    ...
    const { error } = await supabase.rpc("transfer_node_wallet",{p_from_wallet_id:from,p_to_wallet_id:to,p_amount:value,p_note:note||"Movimiento interno"});
  ...
  return <main className="wallet-shell"><header className="wallet-top"><div><div className="eyebrow">SISTEMA INTERNO · MODO TEST</div><h1>TORRE DE LOS MAGOS</h1><p>Wallet de movimientos entre nodos</p></div><div className="secure">● PRUEBAS · CORREO DESACTIVADO</div></header>
    <section className="wallet-wrap"><div className="hero-row"><div><span className="label">BALANCE CONSOLIDADO</span><div className="total">{money(total)}</div><span className="muted">Saldos persistentes en Supabase</span></div><button className="ghost" onClick={load}>Actualizar</button></div>
