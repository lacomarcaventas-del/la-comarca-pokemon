...
export default function Home(){return <div className="siteShell">
  ...
  <main>
    <section id="lineas" className="wrap sectionBlock"><div className="sectionTitle"><h2>Explora nuestras líneas</h2><span>La Comarca · TCG & Hobby</span></div><div className="lineGrid">{lines.map(([name,key,category])=><Link href={`/catalogo?categoria=${encodeURIComponent(category)}`} className={`lineCard ${key}`} key={key}><div className="lineMark">{name}</div><strong>Explorar <span>→</span></strong></Link>)}</div></section>
